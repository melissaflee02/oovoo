...
import { getCommunitiesFromFirestore } from '../../firebase';
import CircularIcon from "../circularicon.js";
import filter from 'lodash.filter';

// https://www.youtube.com/watch?v=Q4S9M9rJAxk
export default function CreateTripSearch({ navigation }) {
  ...
  useEffect(() => {
    async function updateData() {
      communities = await getCommunitiesFromFirestore();
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity onPress={() => navigation.push('Create Trip', { community: item })} style={ styles.communityContainer }>
